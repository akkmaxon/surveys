...
q_1 = Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 1,
  title: "Прочитайте высказывания, которые описывают противоположные мнения работников о себе и своей компании. C какой из групп Вы более согласны? Отметьте это на шкале: 1 - полностью согласен с мнением слева, 6 - полностью согласен с мнением справа.",
...
LeftStatement.create! do |left|
  ...
  left.text = "Я четко выполняю свои обязанности, согласно должностной инструкции, не больше и не меньше. Я не склонен проявлять инициативу, считая, что на работе она неуместна, и часто наказуема."
end
...
LeftStatement.create! do |left|
  ...
  left.title = "Инициатива убивает систему"
  left.text = "Большинство моих коллег четко выполняют свои обязанности, согласно должностной инструкции, не больше и не меньше. Они не склонны проявлять инициативу, считая, что на работе она неуместна, и часто наказуема."
end
RightStatement.create! do |right|
  ...
  Они готовы брать ответственность за реализацию предложений на себя."
end
### 3 question ###
...
LeftStatement.create! do |left|
  ...
  left.text = "Я работают так, как меня когда-то научили. Я убежден, что не надо ничего менять, если каждый будет делать по-своему, в компании начнется беспорядок."
end
RightStatement.create! do |right|
  right.question_id = q_3.id
  right.title = "Постоянно улучшая"
  right.text = "Я считаю, что моей задачей является постоянное улучшение рабочих процессов. Я часто вношу изменения в свою работу, чтобы сделать ее проще, удобнее. После выполнения какого-либо задания, я не могу его забыть, пока не продумаю, как можно было бы сделать его лучше."
end
### 4 question ###
...
LeftStatement.create! do |left|
  ...
  left.title = "Индивидуальный игрок"
  left.text = '"Совместная работа, - считает большинство моих коллег, - приводит к тому, что кто-то работает больше, а кто-то меньше. Кто-то может вообще подвести всех". Предпочитая работать в одиночку, такие работники добиваются больше, чем работая в команде и пытаясь справедливо распределить работу между ее членами.'
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.title = "Начальник"
  left.text = "Мне не нравится работать с моим непосредственным руководителем по разным причинам. Но не оказывает мне никакой поддержки, может грубо разговаривать, не решает проблемы, а только требует."
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.text = "Скучная и неинтересная работа приводит к тому, что время течет слишком медленно для меня. Я с охотой поменял бы эту работу на что-то более интересное."
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.title = "Работа как работа"
  left.text = "Скучная и неинтересная работа приводит к тому, что время течет слишком медленно для большинства моих коллег. Они бы с охотой поменяли эту работу на что-то более интересное."
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.text = "Я считаю, что клиенты требуют всегда больше, чем положено. В идеале я бы вообще не хотел работать напрямую с жителями. Меня раздражают вечные жалобы и недовольство."
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.text = "Я не рекомендую свои друзьям, ищущим работу, идти на работу в нашу компанию. В разговорах я иногда стесняюсь говорить, что работаю здесь. Компания в целом имеет не очень хорошую репутацию в городе, и в нее неохотно идут новые сотрудники."
end
RightStatement.create! do |right|
...
RightStatement.create! do |right|
  ...
  right.title = "Все есть для работы"
  right.text = "Большинство моих коллег довольны и зарплатой, и условиями труда. У них удобное рабочее место и достаточное обеспечение всем необходимым для работы."
end
### 23 question ###
...
LeftStatement.create! do |left|
  ...
  left.text = "Коллектив, в котором я работаю, трудно назвать дружелюбным. Все тайно радуются, если коллега совершит ошибку. Я редко хожу на корпоративные мероприятия, так как считаю это пустой тратой времени."
end
RightStatement.create! do |right|
...
LeftStatement.create! do |left|
  ...
  left.text = "Я считаю себя профессионалом, но в мои дела постоянно вмешиваются, указывая, как работать. Мне редко доверяют серьезные участки работы, показывая, что не совсем доверяют. Мне неприятно, что меня считают некомпетентным. Если бы мне больше доверяли, я бы мог показать лучшие результаты."
end
RightStatement.create! do |right|
...
### 2_1 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 201,
  sentence: "Я считаю, что основными проблемами в компании являются ...",
  criterion: questions_2_criterion
### 2_2 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 202,
  sentence: "Если бы я был назначен генеральным директором своей компании, в первую очередь я изменил бы ...",
  criterion: questions_2_criterion
### 2_3 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 203,
  sentence: "Я чувствовал гордость за свою компанию, когда ...",
  criterion: questions_2_criterion
### 2_4 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 204,
  sentence: "Чтобы работники компании работали с большей душой и отдачей, руководству компании нужно сделать следующее ...",
  criterion: questions_2_criterion
### 2_5 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 205,
  sentence: "В целом я посоветовал бы нашему руководству ...",
  criterion: questions_2_criterion
### 2_6 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 206,
  sentence: "Назовите 2-3 имени (ФИО) Ваших коллег, точку зрения которых Вы уважаете, к мнению которых прислушиваетесь:",
  criterion: questions_2_criterion
### 2_7 question ###
Question.create! opinion_subject: "Я", audience: "Менеджмент", number: 207,
